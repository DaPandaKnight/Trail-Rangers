...
// ── LINZ REQUEST QUEUE ───────────────────────────────────────────────────
// API Gateway can return a transient 503 before the Lambda is invoked when
// MapLibre starts raster, vector and font requests together. MapLibre's
// image-request limit does not include PBF requests, so queue *all* proxied
// resources here. The Lambda remains responsible for LINZ-origin retries.
const PROXY_PROTOCOL = 'linzproxy';
const MAX_PARALLEL_PROXY_REQUESTS = 2;
...
async function fetchProxiedResource(params, abortController) {
  const realUrl = params.url.replace(`${PROXY_PROTOCOL}://`, '');
  let lastError;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (params.type === 'json') {
          ...
          // MapLibre decodes image responses itself and expects their bytes.
      ...
      lastError = new Error(
        `Proxy request failed (${response.status}): ${realUrl}`
      );
      lastError.retryable = [429, 503].includes(response.status);
      ...
      await waitForRetry(retryDelay(response, attempt), abortController.signal);
    } catch (error) {
      ...
      lastError = error;
      if (!error.retryable || attempt === MAX_PROXY_RETRIES) {
        throw lastError;
      }
...
function drainProxyQueue() {
  while (
    activeProxyRequests < MAX_PARALLEL_PROXY_REQUESTS &&
    pendingProxyRequests.length
  ) {
    const job = pendingProxyRequests.shift();
    if (job.cancelled) {
      continue;
    }
    ...
    fetchProxiedResource(job.params, job.abortController)
      .then(job.resolve, job.reject)
      .finally(() => {
        job.abortController.signal.removeEventListener(
          'abort',
          job.onAbort
        );
        activeProxyRequests--;
...
maplibregl.addProtocol(PROXY_PROTOCOL, (params, abortController) =>
  new Promise((resolve, reject) => {
    ...
    job.onAbort = () => {
      ...
      job.cancelled = true;
      const index = pendingProxyRequests.indexOf(job);
      if (index !== -1) {
        pendingProxyRequests.splice(index, 1);
      }
      reject(createAbortError());
    };
    ...
    abortController.signal.addEventListener('abort', job.onAbort, { once: true });
    pendingProxyRequests.push(job);
    drainProxyQueue();
  })
);
...
function transformRequest(url, resourceType) {
  ...
  // Queue direct calls to this proxy too (aerial tiles, sprites and glyphs).
  if (url.startsWith(`${API_BASE}/proxy/`)) {
    return { url: `${PROXY_PROTOCOL}://${url}` };
  }
  ...
  if (url.startsWith(LINZ_ORIGIN)) {
    ...
    return {
      url: `${PROXY_PROTOCOL}://${API_BASE}/proxy/${path}${parsed.search}`
    };
  }
...
if (
  ...
  'function') {
  // API Gateway can transiently reject tile bursts before the Lambda runs.
  // Two concurrent image requests keeps terrain tiles below that threshold.
...
// ── TILE ERROR HANDLING ────────────────────────────────────────────────
// Keep application-console output clean if a request ultimately fails after
// queueing and retrying. The browser Network panel will still correctly show
// a real HTTP failure; JavaScript cannot and should not hide that diagnostic.

let tileErrorCount = 0;
let tileErrorSummaryTimer = null;
let tileErrorToastEl = null;
let tileErrorHideTimer = null;

function showTileErrorToast() {
  if (!tileErrorToastEl) {
    tileErrorToastEl = document.createElement('div');
    tileErrorToastEl.textContent = 'Some map details are taking longer than usual to load…';
    Object.assign(tileErrorToastEl.style, {
      position: 'fixed',
      bottom: '20px',
      left: '50%',
    ...
    document.body.appendChild(tileErrorToastEl);
  }

  tileErrorToastEl.style.opacity = '1';
  clearTimeout(tileErrorHideTimer);
  tileErrorHideTimer = setTimeout(() => {
    tileErrorToastEl.style.opacity = '0';
  }, 3000);
}
...
map.on('error', e => {
  ...
  // Cancelled requests are expected during fast panning/zooming, not
  // real failures — MapLibre already discards these silently elsewhere,
  // but skip them here too in case that ever changes.
  if (err && err.name === 'AbortError') return;

  tileErrorCount++;

  // Reset the burst counter quietly. The toast below is enough feedback for
  // a visitor, and avoids adding another warning to the developer console.
  clearTimeout(tileErrorSummaryTimer);
  tileErrorSummaryTimer = setTimeout(() => {
    tileErrorCount = 0;
  }, 500);

  // Only bother the user with a visible notice once failures start
  // piling up, not for a single stray tile.
  if (tileErrorCount >= 4) {
    showTileErrorToast();
  }
});
...
map.on('load', async () => {
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      resp = await fetch(TOPO_STYLE_URL);
      if (resp.ok || attempt === MAX_RETRIES) break;
